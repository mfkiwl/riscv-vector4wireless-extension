...
{ 
  tmp = A[segIdx[i]] * x[i]; //SEW = SEW * SEW
  tmp = alg_round_shift_right(tmp, n) + B[segIdx[i]]; //SEW = SEW + SEW, n depend on customer implementation
...
{ 
  tmp = A[segIdx[i]] * x[i]; //SEW = SEW * SEW
  tmp = alg_round_shift_right((A[segIdx[i]] * x[i]), n) + B[segIdx[i]]; //SEW = SEW + SEW, n depend on customer implementation
  e[i], E[i] = expAdjust(R_NL_TYPE, E[i]);
  vd[j][i] = postProc(R_NL_TYPE, tmp, s[i], E[i]);
}
----
