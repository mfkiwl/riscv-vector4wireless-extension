...
=== Introduction
In the fast nonlinear instructions vs2 store the fixed-point value in Q32.n format，vs1 store the point position n of the fix point format, n belongs to [0, 31]. vd stores the scaling shift value n of output or the mantissa value of the output in Q32.n format. +
...
R_NL_SEG_EXP[16]: 16 nonlinear function segment points exponent registers (required for some nonlinear functions) R_NL_SEG_EXP[0] stores exponent of R_NL_A and R_NL_B, R_NL_SEG_EXP[1] stores exponent of expected intermediate output +
