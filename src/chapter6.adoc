...
----
VV version
Tmp.r = vs1[j][i].r * vs2[j][i].r - vs1[j][i].i * vs2[j][i].i  // SEW = SEW/2 * SEW/2 - SEW/2 * SEW/2
Tmp.i = vs1[j][i].r * vs2[j][i].i + vs1[j][i].i * vs2[j][i].r  // SEW = SEW/2 * SEW/2 + SEW/2 * SEW/2
vd[j][i].r = clip_low_SEW_SEW2(alg_round_shift_right(Tmp.r, mulsft)) //SEW/2=SEW
vd[j][i].i = clip_low_SEW_SEW2(alg_round_shift_right(Tmp.i, mulsft)) //SEW/2=SEW

VS version
Tmp.r = vs1[0][0].r * vs2[j][i].r - vs1[0][0].i * vs2[j][i].i  // SEW = SEW/2 * SEW/2 - SEW/2 * SEW/2
Tmp.i = vs1[0][0].r * vs2[j][i].i + vs1[0][0].i * vs2[j][i].r  // SEW = SEW/2 * SEW/2 + SEW/2 * SEW/2
vd[j][i].r = clip_low_SEW_SEW2(alg_round_shift_right(Tmp.r, mulsft)) //SEW/2=SEW
vd[j][i].i = clip_low_SEW_SEW2(alg_round_shift_right(Tmp.i, mulsft)) //SEW/2=SEW
----
...
. *Description*
+
The real and imaginary parts of the complex elements in vs2 are accumulated separately and the accumulated value is shifted and the low SEW bit is intercepted and output to vd[0]. When SEW is configured as 32bit, SC16 complex addition is performed. not controlled by LMUL, perform operations on a single VR statically.
...
Tmp.i = 0；
For each i in vs2 
   Tmp.r += v0.b[i] == 0 ? 0 : vs2[0][i].r; //2SEW = SEW
   Tmp.i += v0.b[i] == 0 ? 0 : vs2[0][i].i; //2SEW = SEW
vd[0][0].r = clip_low_2SEW_SEW(alg_round_shift_right(Tmp.r, accsft)) // SEW = 2SEW
vd[0][0].i = clip_low_2SEW_SEW(alg_round_shift_right(Tmp.i, accsft)) // SEW = 2SEW
----
...
. *Description*
+
The signed fixed-point complex numbers of vs2 and vs1/vs1[0] are multiplied. The intermediate result after the multiplication retains 2 times the bit width, and then performs arithmetic shifting, and the result is accumulated with the value in the accumulation register.  When SEW is 32bit, SC16 complex multiplication operation is performed.
...
VV version
Tmp.r = vs1[j][i].r * vs2[j][i].r - vs1[j][i].i * vs2[j][i].i  // SEW = SEW/2 * SEW/2 - SEW/2 * SEW/2
Tmp.r = alg_round_shift_right(Tmp.r, MULSFT[j][i]);
Tmp.i = vs1[j][i].r * vs2[j][i].i + vs1[j][i].i * vs2[j][i].r  // SEW = SEW/2 * SEW/2 + SEW/2 * SEW/2
Tmp.i = alg_round_shift_right(Tmp.i, MULSFT[j][i]);
ACCREG[i].r += Tmp.r;
ACCREG[i].i += Tmp.i;
...
. *Description*
+
The complex numbers of vs2 and vs1 and vs1[0] are multiplied. The intermediate result after the multiplication retains 2 times the bit width, and then performs an arithmetic right shift, and the result is accumulated with the value in the accumulation register. Arithmetically shift the real part and imaginary part of the value in the accumulation register, truncate and output them to vd, and clear the accumulation register. When SEW is 32bit, the complex number of SC16 is output. *Clear the ACCREG after all element of the VR are processed.*
...
ACCREG[i].r += Tmp.r
ACCREG[i].i += Tmp.i
vd[0][i].r = clip_low_40_SEW2(alg_round_shift_right(ACCREG[i].r, accsft)) // SEW/2 = ACCREG_LEN
vd[0][i].i = clip_low_40_SEW2(alg_round_shift_right(ACCREG[i].i, accsft)) // SEW/2 = ACCREG_LEN
ACCREG[i] = 0
...
ACCREG[i].r += Tmp.r
ACCREG[i].i += Tmp.i
vd[0][i].r = clip_low_40_SEW2(alg_round_shift_right(ACCREG[i].r, accsft)) // SEW/2 = 40bits
vd[0][i].i = clip_low_40_SEW2(alg_round_shift_right(ACCREG[i].i, accsft)) // SEW/2 = 40bits
ACCREG[i] = 0
----
...
. *Description*
+
The complex conjugate of vs1/vs1[0] and vs2 is multiplied. The intermediate result after the multiplication retains 2 times the bit width, and then performs an arithmetic right shift, and the result is accumulated with the value in the accumulation register. Arithmetically shift the real part and imaginary part of the value in the accumulation register, truncate and output them to vd, and clear the accumulation register. When SEW is 32bit, the complex number of SC16 is output. *Clear the ACCREG after all element of the VR are processed.*
...
. *Description*
+
The complex numbers of vs2 and vs1/vs1[0] are multiplied. The intermediate result after the multiplication retains 2 times the bit width, and then performs an arithmetic right shift, and the result is accumulated with the value in the accumulation register. Perform arithmetic shifts on the real part in the accumulation register, intercept the low bits of the real part, and output them to vd, and clear the real part to zero. When SEW is configured as 32bit, the real part of the output is 32bit. This command is invalid when SEW is configured as other values. *Clear the ACCREG all real part after all element of the VR are processed.*
...
Tmp.r = alg_round_shift_right(Tmp.r, MULSFT[j][i])
ACCREG[i].r += Tmp.r
vd[0][i] = clip_low_40_SEW(alg_round_shift_right(ACCREG[i].r, accsft)) // SEW = 40bits
ACCREG[i] = 0
...
. *Description*
+
Multiply the complex numbers of vs2 and vs1/vs1[0], perform an arithmetic shift on the imaginary part in the accumulation register, intercept the low bits of the imaginary part and output it to vd, and clear the imaginary part to zero. When SEW is configured for 32bit, the imaginary part of the output is 32bit. This command is invalid when SEW is configured to other values. *Clear the ACCREG all image part after all element of the VR are processed.*
...
Tmp.i = alg_round_shift_right(Tmp.i, MULSFT[j][i])
ACCREG[i].i += Tmp.i
vd[0][i] = clip_low_40_SEW(alg_round_shift_right(ACCREG[i].i, accsft)) // SEW = 40bits
ACCREG[i] = 0
...
vd[i] = alg_round_shift_right(Tmp.r, mulsft) //SEW
...
vd[i] = alg_round_shift_right(Tmp.i, mulsft) //SEW
