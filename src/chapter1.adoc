...
- min: Find the minimum value from vector registers from right to left, from small VR index to high VR index. the right element and lower VR index have the higher priority to be the minimum value if there are multiple element equal to minimum value.
- max: Find the maximum value from vector registers from right to left, from small VR index to high VR index. the right element and lower VR index have the higher priority to be the maximum value if there are multiple element equal to minimum value.
...
- round_clip_high_x: Saturate clip the high SEW width bit number with vxrm mode rounding. e.g. SEW=32bit, round_clip_high_SEW(0x0050000381) = 0x00500004, round_clip_high_SEW(0x7fffffff80) = 0x7fffffff
