...
- ‘pa2c’ is CPF(6/6/4)->SC16 format conversion； 
- ‘pb2c’ is CPF(7/7/2)->SC16 format conversion.

. *Operation*
+
----
vd[j][i] = loadFormatConvert(void *rs1, A2B) {
  switch(A2B)
    case ‘b2h’
      return sx16(int8 rs1[j][i])
    case ‘b2w’
      return sx32(int8 rs1[j][i])
    case ‘h2w’
      return sx32(int16 rs1[j][i])
    case ‘pa2c’
      img = get_i(cfl rs1[j][i])
      real = get_r(cfl rs1[j][i])
      e = get_e(cfl rs1[j][i])
      I = zp32( img )
      R = zp32( real )
      return vcpack(alg_round_shift_right(I, e), alg_round_shift_right(R, e))
    case 'pb2c'
      // support in future
}
----
...
- ‘c2pa’ is SC16->CPF(6/6/4) format conversion； 
- ‘c2pb’ is SC16->CPF(7/7/2) format conversion.

. *Operation*
+
----
void storeFormatConvert(void *rs1, vs3[j][i], B2A) {
  switch(B2A)
    case ‘h2b’
      vmem(clip_low_16_8(int16 vs3[j][i]), (int8*)rs1[j][i])
    case ‘w2b’
      vmem(clip_low_32_8(int32 vs3[j][i]), (int8*)rs1[j][i])
    case ‘w2h’
      vmem(clip_low_32_16(int32 vs3[j][i]), (int16*)rs1[j][i])
    case ‘c2pa’
      I = vs3[j][i].i
      R = vs3[j][i].r
      num = abs(I) > abs(R) ? abs(I) : abs(R)
      exp = lsb(num)
      img = round_clip_high_16_6(I << exp)
      real = round_clip_high_16_6(R << exp)
      vmem(cfp_pack(exp, img, real), (int16*)rs1[j][i])
   case 'c2pb'
      // support in future
}
----
