...
extern bool const_xcls_imm8_operand (rtx, machine_mode);
extern bool const_xcls_imm9_operand (rtx, machine_mode);
extern bool const_xcls_imm10_operand (rtx, machine_mode);
extern bool mem_pre_inc (rtx, machine_mode);
extern bool mem_post_inc (rtx, machine_mode);
extern bool mem_plus_reg (rtx, machine_mode);
extern bool const_xcpa_shf_operand (rtx, machine_mode);
...
extern bool const_int8s_operand (rtx, machine_mode);
...
extern bool nonimmediate_nonpostinc (rtx, machine_mode);
...
{
  return c >= CONSTRAINT_w && c <= CONSTRAINT_th__m__miu;
}
				      ...
				      bool *allows_reg, bool *allows_mem)
{
  if (c >= CONSTRAINT_c01 && c <= CONSTRAINT_CV__simd__i03)
    return;
  if (c >= CONSTRAINT_Wlr && c <= CONSTRAINT_Wlr)
    {
      *allows_reg = true;
      return;
    }
  if (c >= CONSTRAINT_V && c <= CONSTRAINT_Wma)
    {
      *allows_mem = true;
      return;
    }
